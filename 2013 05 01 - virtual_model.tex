...
\begin{abstract}
This paper describes methods of analysing the structure of modules of the
discrete-event system in order to achieve modular diagnosability of the system.
A non-compositional analysis of the system is introduced as a preliminary step
for the process of verification of modular diagnosability property, in order to
to reduce the overall computational expenses.
...
This paper considers a DES with modular structure and investigates how to
compose its modules in order to force the modular diagnosability
property of the system.
For this goal a non-compositional analysis of the system is introduced.
...
correspondent automaton. As a starting point we consider the definition of
modular diagnosability property and the correspondent verification algorithm
introduced in \cite{contant_diagnosability_2006}.

(Remake this paragraph) This paper is organized as follows. Section
\ref{sec:Preliminaries} covers the necessary notation and describes the diagnosability problem. Section
...
specifications. The approaches are also known as \emph{event-based} and
\emph{state-based} correspondingly.

In the event-based approach fault events are a special type of event, such
that $\Sigma_{uo}$ can be disjointed into the sets of faults $\Sigma_f$ and
non-faults $\Sigma\backslash \Sigma_f$. A string containing a fault event is
called \emph{faulty string}. A set of faulty
strings is called \emph{faulty language}, i.e. formally 
$$L_f = \{ s \in L \mid \sigma \in s, \sigma \in \Sigma_f\}.$$
By definition, the faulty language is not prefix-closed in general, $L_f
\subseteq \overline{L_f}$. Thus, in the event-based approach the language of
the system can be partitioned into faulty and non-faulty languages, where
the \emph{non-faulty language} is defined as $L_{nf} = L \backslash L_f$.
...
In the case of the state-based approach the faulty specification allows us to
define undesired behavior when the fault events are not necessarily introduced.
In this case this behaviour is represented by a marked language $L_f = L_m
\subseteq L$.
In this paper we assume that the faulty language can be defined over the
language of any model by either the event-based or state-based approach, for the
sake of unification of definitions.
...
faulty specifications for the same language. For the sake of simplicity we
assume that there is only one type of fault such that it is not necessary to
identify which type of fault occurred.

We define \emph{diagnosability of the fault} as follows:
\begin{definition} The fault is \emph{not
diagnosable} if there are two strings with same observation such that one string
is faulty and of infinite length, and second one is non-faulty, i.e. if the
...
We recall the observation in \cite{contant_diagnosability_2006} that the global
language of the system is not diagnosable only if exists at least one
non-diagnosable local language.
If all the local languages are diagnosable then the global language is
diagnosable. This statement is proved by Theorem 2, Part 2 in
...
$\{L_{i \in \mathbb{N}}\}$. The global language $L = \parallel L_i$ is
\emph{locally diagnosable} if each local language $L_i$ is diagnosable, i.e. if
the following holds true:
\end{definition}
\begin{equation}
\begin{array}{l}
	(\forall L_i \in \{L_{i \in \mathbb{N}}\}) \\
	(\not \exists s \in L_{i,f}, t \in L_{i,nf})(|s| = \infty)
	[M(s) = M(t)].
\end{array}
\end{equation}

Thus, in our definition of modular diagnosability we focus only on the case when
the global language is not locally diagnosable.

\begin{definition}[Modular diagnosability] Given the set of local languages
$\{L_{i \in \mathbb{N}}\}$ and its corresponded sets $\{L_{i,f}\}$ and
$\{L_{i,nf}\}$. If the global language $L = \parallel L_i$ is not locally
diagnosable then it is \emph{modularly diagnosable} if all the local languages
are diagnosable with respect to their local observations, i.e. if the following
holds true:
\end{definition}
\begin{equation}
\begin{array}{l}
	(\forall L_i \in \{L_{i \in \mathbb{N}}\} : \\ 
	(\exists s \in L_{i,f}, t \in L_{i,nf})(|s| = \infty)(M(s) = M(t)) \\
	\left[M_i(M_i^{-1}(s))= M_i(M_i^{-1}(t))\right].
\end{array}
\end{equation}
