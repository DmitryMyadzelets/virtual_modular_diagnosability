...
modules. A structural analysis of the system is introduced, as a
...
presented in \cite{sampath_diagnosability_1995}, and its complexity was 
improved in \cite{jiang_polynomial_2001} and \cite{yoo_polynomial-time_2002}.
...
\cite{contant_diagnosability_2006}. We review the all approaches in the next
section.
...
assume that the systems' modules are given by its designer. The design may
reflect a physical or functional structure of the system, or have other
underlining design principals, which are \emph{natural} for the designer. Thus,
we assume that preserving the systems' modularity as close to the initial
...
system is defined by the parallel composition of its local languages: $L
...
A DES is presented by one module, and it is assumed that all the observations
and diagnosis can be performed at one site. The structure is depicted in Figure
\ref{fig_centralized}. In this architecture the global language is built by the
parallel composition and then the diagnoser $D$
...
This approach exploits the entire model built from its modules. The several
local sites performing observations, and only local diagnosers are built. The
diagnosers do not communicate to each other, but provide necessary information
(a protocol) to a central decision node. This architecture is depicted in Figure
\ref{fig_decentralized}.
...
is possible to apply for the given system. The notion of modular diagnosability
meets the same architectural implications, and we refer it as to the distributed
diagnosability when the observation spots do not communicate to each other.
...
can be considered as a \emph{virtual module}, and the system with the new
modularity satisfies the property of modular diagnosability.
...
% \subsection{}
% \begin{enumerate}
%   \item Do not consider structural analysis first;
...
%   no composition involved (structural analysis)?
...
% % structural analysis should be the fist necessary step
...
% % referred as to the structural analysis. The step can be describe as
...
% % 
% % The structural analysis equal to the above is presented in the work
...
% % The cases (a) and (b) can be referred as to the structural analysis, and
...
% % 
% % It can be seen from the above, that the structural analysis has
% % been limited only for checking of indeterminate cycle. In the following we
% % extend the structural analysis and provide additional conditions for
...
% \end{corollary}
% 
% \begin{example} This example shows how the structural analysis might be
% organised.
...
% 
% A basic idea for structural analysis of two modules is to create a
