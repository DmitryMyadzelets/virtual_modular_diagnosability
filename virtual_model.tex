...
% \subsection{Problem statement - diagnosability (Informally about centralized
%  and decentralized}
...
%   \item The order of composition can be tuned to achieve lower computational
%   expenses.
% \end{enumerate}
% 
...
following: $P_i(L) = \{s\mid s\in L_{i}\}$ and $P_i^{-1}(L_{i})
= \{s' \mid s' \in L\}, ~i \in I$.
...
% $L_2$ are coupled (synchronised). The subset of $L_1$, all the strings which
% have common events in it, is defined as:
% \begin{equation}
% \label{eq_synchronized_language}
% 	\{s \in L_1 \mid  \sigma \in s, \sigma \in \Sigma_1 \cap \Sigma_2 \}.
% \end{equation}
...
% An example of automata reflecting the partitioning of a language is depicted in
...
% Since the faulty language is not prefix-closed in general, the marked language
% is used to distinguish it from non-faulty language.
...
\begin{array}{l}
	...
	(|t| \geq n) 
	\\
% 	\left[ M^{-1}(M(st)) \cap L_{nf} = \emptyset \right].
...
\begin{array}{l}
	...
	(|t| \geq n)
	\\
% 	\left[ M_i^{-1}(M_i(st)) \cap L_{i,nf} = \emptyset \right].
...
\begin{array}{l}
	...
	(|t| \geq n)
% 	\\
% 	\left[ M_i^{-1}(M_i(st)) \cap L_{i,nf} = \emptyset \right] \lor
	\\
% 	\left[ M_i^{-1}(M_i(P_i^{-1}(st))) \cap P_i^{-1}(L_{i,nf}) = \emptyset \right].
...
\begin{definition}[Diagnosability of virtual modules] Let $I = \{1,2,\ldots,n\}
...
$J$ is exponential with respect to the cardinality of $I$. However, not
...
Given two languages $L_1$ and $L_2$, and a string $s \in L_{i \in \{1,2\}}$.
Assume the string is not blocked by the composition, i.e. $s \in P_i(L_1
\parallel L_2)$. The string $s$ changes its observation in the language $L_1
\parallel L_2$ if and only if:
\begin{subequations}\label{lem:obs}
\begin{align}
	(\exists \sigma \in s \mid \sigma \in \Sigma_i \cap \Sigma_j, j \neq i)
	\label{lem:obs1}
	\\
...
% 	(\forall j \in \{1,2\}, j \neq i)
% 	\\
% 	(\forall t\sigma \in L_j 
% 	\mid \sigma \in s, \sigma \in \Sigma_i \cap \Sigma_j) 
% 	\\
% 	\left[ M_j(t) \backslash \{\epsilon\} \neq \emptyset \right], \textrm{where } 
% 	\\
% 	M_j: \Sigma^* \rightarrow (\Sigma_{j,o} \backslash \Sigma_i)^*.
\end{align}
\end{subequations}
$$
...
necessary.
Assume $\exists s' \in L_1 \parallel L_2$ and $\exists s \in L_i$ such that
$M(s')= M_i(s)$ and, hanceforce, (\ref{def:obs}) is false. Let $\exists
\sigma \in \Sigma_i \cap \Sigma_j$ such that $\sigma \in s'$ and also
(\ref{lem:obs1}) holds. Then may $\exists t'\sigma \in \overline{s'}$ such that
$M_{I\backslash i}(t') = \emptyset$, and then $M_{I\backslash i}(P_j(t')) =
\emptyset$ which contradicts (\ref{lem:obs2}). Now, let (\ref{lem:obs2})
be true for all $t\sigma \in \overline{P_j(s')}$. Then the assumption $M(s')=
M_i(s)$ holds only if $\sigma \not \in s$, which contradicts (\ref{lem:obs1}). 

We prove necessity of (\ref{lem:obs}) by contradiction.
Let (\ref{lem:obs1}) holds, and $\exists t\sigma \in \overline{L_j}$ such that
$M_{I\backslash i}(t) = \emptyset$. Then may $\exists t'\sigma \in 
L_1 \parallel L_2 \subseteq P_j^{-1}(t\sigma)$ such that $M_{I\backslash i}(t') =
\emptyset$ and $M_i(t') = M_i(s) \neq \emptyset$, which contradicts (\ref{def:obs}).
Now, let (\ref{lem:obs2}) holds and $\not \exists \sigma \in s' \in P_i^{-1}(s)
\mid \sigma \in \Sigma_i \cap \Sigma_j$. Then may $\exists w \in L_j$ and,
hance, $w' \in L_1 \parallel L_2 \subseteq P_j^{-1}(w)$ such that $M(w')=M(s)$,
wich contradicts (\ref{def:obs}).
% $$
% \begin{array}{l}
% \textrm{Sufficiency, by induction:}\\
% \exists t\sigma \in L_j \mid M_j(t) \neq \emptyset 
% \Rightarrow \\
% (\forall t'\sigma \in P_j^{-1}(t\sigma))(M_j(t'\sigma) \neq \emptyset)
% \Rightarrow \\
% (\forall s' \in P_i^{-1}(s) \mid s' \in P_j^{-1}(t\sigma))(M_j(s')\neq
% 	\emptyset).
% \\\\
% \textrm{Necessity, by contradiction:}\\
% \textrm{Let } \exists t\sigma \in L_j \mid M_j(t) = \emptyset \Rightarrow \\
% \textrm{may } \exists s' \in P_j^{-1}(t\sigma) \cap P_i^{-1}(s\sigma) \\
% \textrm{s.t. } M_j(P_i(s')) = M_i(s') \\
% \textrm{which contradicts the definition.}  
% \end{array}
% $$ 
\end{proof}

We call the set of stings $t \in \overline{L_j}$ satisfying condition
(\ref{lem:obs2}) as an \emph{adjacent observable support} for the given
string $s \in L_i$.

\begin{definition} Given two languages $L_1$ and $L_2$. We say that a string $s
\in L_{i \in \{1,2\}}$ is distinguished from all the other local strings
$L_i\backslash s$ in the language $L_1 \parallel L_2$ if the following holds
true:
\end{definition}
\begin{equation}
\label{def:dist}
\begin{array}{l}
	...
	\left[M(w \parallel L_j) \cap M(s\parallel L_j) = \emptyset\right].
\end{array}
\end{equation}

\begin{lemma}
\label{lem_distinguished}
Given two languages $L_1$ and $L_2$. Assume no strings are blocked by the
composition, i.e. $L_i = P_i(L_1 \parallel L_2)$. The string $s \in L_{i \in
\{1,2\}}$ is distinguished from $L_i\backslash s$ in the language $L_1 \parallel
L_2$ if $s$ has an adjacent observable support $L_{j,s} \subseteq L_j$ wich
satisfies the following condition:
\end{lemma}
\begin{subequations}
\begin{align}
	(\forall t \in L_{j,s})
	(\exists \sigma \in t \mid \sigma \in \Sigma_i \cap \Sigma_j, j \neq i)
	\label{lem:dist1}
	\\
	(\forall w \in L_i\backslash s)(\sigma \not \in w)
	\label{lem:dist2}
	\\
	(\forall t'\sigma \in \overline{t})
	[M_{I\backslash i}(t\backslash t'\sigma) \neq \emptyset]
	\label{lem:dist3}
\end{align}
\end{subequations}

\begin{proof}
Assume (\ref{def:dist}) is false, i.e. $\exists w' \in P_i^{-1}(w)$ and $\exists
s' \in P_i^{-1}(s)$ such that $M(w') = M(s')$. Assume (\ref{lem:dist1}) and
(\ref{lem:dist2}) hold. Then may $\exists t' \in \overline{s'} \mid t' \in
P_j^{-1}(L_{j,s})$ such that $M(t') = M(w')$, and $t = P_j(t')$ such that
$M_{I\backslash i}(t) = M_{I\backslash i}(w)$. And may $\exists t'' \in
\overline{t}$ such that $M_{I\backslash i}(t'') = M_{I\backslash i}(w)$.  Since
$\sigma \in t$ and $\sigma \not \in w$, then for any $t''\sigma \in
\overline{t}$ $M(t \backslash t''\sigma) = \emptyset$ which contradicts
\ref{lem:dist3}.

The proof continues to state sufficiency of (\ref{lem:dist2}) and
(\ref{lem:dist3}).
\end{proof}
