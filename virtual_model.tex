...
commone events. Thus, whenever we observe those events of $L_2$,
...
can be change either by blocking the string in the local language due to
concurrency or by inteleaving with obsevable events in other languages. Under
assumption that all the strings are not affected by concurrency, i.e. $L_1 =
P_1(L)$ we can state that conditions of Lemma \ref{lem:distinguished} are also
necessary for changing distinguishability. For the sake of brevity no proof is
provided here.
